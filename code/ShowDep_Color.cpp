...
// 此函数创建一个表，用于将深度摄像机坐标映射到3D世界坐标。
// 它接受校准信息和用于输出XY表的图像。
static void create_xy_table(const k4a_calibration_t* calibration, k4a_image_t xy_table)
{
	// 将图像缓冲区强制转换为k4a_float2_t以便更容易操作
...
// 此函数生成点云数据，将深度图映射到3D点云。
// 它接受深度图、XY表和用于输出的点云图像，同时返回有效点的数量。
static void generate_point_cloud(const k4a::image depth_image, const k4a_image_t xy_table, k4a_image_t point_cloud, int* point_count)
{
	...
	*point_count = 0;
	...
	for (int i = 0; i < width * height; i++)
	{
		...
		{
			// 如果深度值为零或XY表中的坐标为NaN，则将点云坐标设置为NaN
...
static void write_point_cloud(const char* file_name, const k4a_image_t point_cloud, int point_count)
{
	...
	// 保存为PLY文件
	//`std::ofstream ofs(file_name);`: 这行代码创建了一个名为 `ofs` 的文件输出流对象，
	// 它与指定的文件名 `file_name` 相关联。如果文件不存在，将会创建一个新文件；如果文件已存在，它将被截断（即清空文件内容）。
	std::ofstream ofs(file_name); // 文本模式
	ofs << "ply" << std::endl;
	ofs << "format ascii 1.0" << std::endl;
	
	// `ofs << "ply" << std::endl; `: 这行代码将字符串 "ply" 写入文件流 `ofs`。
	// `< < ` 运算符用于将数据插入流中。`std::endl` 代表插入一个换行符，并刷新缓冲区。这一行的效果是将 "ply" 字符串写入文件并换行。
	...
	//std::ios::app 表示以追加方式打开文件，即在文件末尾追加内容而不覆盖已有内容
}
